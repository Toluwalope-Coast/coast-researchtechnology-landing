...
const TestimonialCard: React.FC<{
  ...
  isActive: boolean;
  height: number; // Add height prop as optional
}> = ({ testimonial, isActive, height }) => {
  return (
    <div
      className={`item text-center ${
        isActive
          ? "opacity-100 transform scale-100"
          : "opacity-10 transform scale-80"
      } transition-all duration-300 ease-in-out`}
      style={{ height }} // Apply height style if provided
    >
      <div className={`${Styles.shadowEffect} bg-white rounded-xl p-4`}>
        ...
        <Image
          ...
          width={90}
          height={90}
          className="rounded-full mx-auto mb-4 border-2 border-background"
        />
      </div>
      <div
        className={`${Styles.testimonialName} ${
          isActive ? "bg-purple-500" : "bg-blue-500"
        } text-white rounded-full py-1 px-4 mt-4 inline-block`}
      >
        {testimonial.name}
      </div>
    </div>
